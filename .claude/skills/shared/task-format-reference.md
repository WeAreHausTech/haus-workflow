...
A **user-thrown gate** is a verification task the user _explicitly asked for in the conversation_ — not a check you invented while decomposing the plan. Typical signals in the user's message: "make sure to verify X before moving on", "add a gate", "run the full end-to-end before closing", "don't proceed until Y is proven", "first on one, then on all".
