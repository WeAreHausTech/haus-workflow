...
try {
  ({ parseMarkdownHeader, buildMarkdownHeader, stampMarkdown, hasHausHeader } = await import(
    "../dist/install/header.js"
  ));
...
describe("header: stampMarkdown", () => {
  ...
  it("replaces existing header, preserving body", () => {
    if (!stampMarkdown) return;
    const original =
      "<!-- HAUS-MANAGED id=x v=1 source=haus@0.0.1 -->\nbody";
...
describe("header: hasHausHeader", () => {
  it("returns true for managed content", () => {
    ...
    assert.ok(
      hasHausHeader("<!-- HAUS-MANAGED id=x v=1 source=y@0.1.0 -->\nfoo"),
    );
  });
...
describe("haus install --dry-run (CLI integration)", () => {
  ...
  it("exits 0 with dry-run output (smoke test)", async () => {
    ...
    const { applyInstall } = await import("../dist/install/apply.js").catch(() => ({ applyInstall: null }));
