...
test('setup-project --json writes scan + recommendation artifacts and exits 0', () => {
  const temp = makeFixture()
  const result = execaSync('node', [path.resolve('dist/cli.js'), 'setup-project', '--json'], {
    cwd: temp,
    reject: false,
  })
...
test('setup-project --json includes recommendation warnings in output', () => {
  const temp = makeFixture()
  execaSync('node', [path.resolve('dist/cli.js'), 'setup-project', '--json'], {
    cwd: temp,
    reject: false,
  })
