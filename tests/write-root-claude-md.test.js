...
test("apply --write creates root CLAUDE.md with import block", () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({ name: "p6-test", packageManager: "yarn@4.5.3", dependencies: { react: "19.0.0" } }, null, 2),
  );
  ...
  assert.equal(rootClaudeMd.includes("@.haus-workflow/haus-way-of-work.md"), true);
  ...
  assert.equal(existsSync(path.join(temp, ".haus-workflow", "haus-way-of-work.md")), true);
  ...
  const wayOfWork = readFileSync(path.join(temp, ".haus-workflow", "haus-way-of-work.md"), "utf8");
  assert.equal(wayOfWork.startsWith("<!-- HAUS-MANAGED id=template.way-of-work"), true);
});
...
test("apply --write is idempotent — second run produces no diff outside markers", () => {
  ...
  writeFileSync(
    path.join(temp, "package.json"),
    JSON.stringify({ name: "p6-idem", packageManager: "yarn@4.5.3", dependencies: { react: "19.0.0" } }, null, 2),
  );
  writeFileSync(path.join(temp, "yarn.lock"), "# lock");
...
test("apply --write skips haus-way-of-work.md when user modified it", () => {
  ...
  const wayOfWorkPath = path.join(temp, ".haus-workflow", "haus-way-of-work.md");
  const original = readFileSync(wayOfWorkPath, "utf8");

  // Simulate user editing the file (keeping the header, modifying body)
  const lines = original.split("\n");
  lines.splice(2, 0, "USER ADDITION: extra line");
  writeFileSync(wayOfWorkPath, lines.join("\n"));

  const result = execaSync("node", [path.resolve("dist/cli.js"), "apply", "--write"], { cwd: temp, reject: false });
  assert.equal(result.exitCode, 0);
  const output = result.stdout + result.stderr;
  assert.equal(output.includes("content modified by user"), true);

  const afterSecondApply = readFileSync(wayOfWorkPath, "utf8");
