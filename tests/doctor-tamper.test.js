...
test('doctor flags a legacy-header WORKFLOW.md whose body has diverged from the template', async () => {
  const dir = mkdtempSync(path.join(os.tmpdir(), 'haus-doctor-tamper-legacy-'))
  const prevCwd = process.cwd()
  const prevExit = process.exitCode
  const prevFixture = process.env.HAUS_FIXTURE_CATALOG
  process.env.HAUS_FIXTURE_CATALOG = path.resolve('tests/fixtures/catalog/manifest.json')
  ...
  try {
    ...
    // Legacy header: no hash= field at all (predates hashing).
    writeFileSync(
      path.join(dir, '.haus-workflow/WORKFLOW.md'),
