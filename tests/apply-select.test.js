...
test('writeClaudeFiles selectedIds: directly exercises the filtering code path via tsx', async () => {
  ...
  const lockEmpty = JSON.parse(
    readFileSync(path.join(temp, '.haus-workflow/haus.lock.json'), 'utf8'),
  )
  assert.equal(
    lockEmpty.length,
    0,
    `expected empty lock with selectedIds=[], got ${lockEmpty.length}`,
  )
  ...
  if (allItems.length > 0) {
    ...
    const lockOne = JSON.parse(
      readFileSync(path.join(temp, '.haus-workflow/haus.lock.json'), 'utf8'),
    )
    assert.equal(
      lockOne.length,
      1,
      `expected 1 lock entry for selectedIds=[${oneId}], got ${lockOne.length}`,
    )
