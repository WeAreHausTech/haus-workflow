...
| `workspace.manifest.json`     | haus     | Written by `haus workspace setup` — a derived, advisory record of each repo's last setup outcome in a multi-repo workspace (see [Workspace mode](./workspace)). Not the workspace's own config — that's `haus.workspace.yaml` at the workspace root, written by `haus workspace discover`/`init` |
...
- **`haus.workspace.yaml`** and **`repos.manifest.json`** — describe a *multi-repo workspace* (which repos exist, how they relate). About **what repos exist**. See [Workspace mode](./workspace) for the full picture; they are not interchangeable with each other either.
