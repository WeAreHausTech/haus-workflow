/**
 * `haus ci-gate` — aggregates `doctor`, `decisions check`, and `update --check --fast`
 * into one documented pass/fail contract for CI, instead of a pipeline needing three
 * separate commands that each independently set `process.exitCode`.
 *
 ...
 * `process.exitCode` isolated to just this call, so aggregating three independent
 * commands doesn't let one's exit code leak into another's, or into the aggregate
 * command's own final exit code before all three have run.
 */
...
export async function runCiGate(options: { json?: boolean } = {}): Promise<void> {
  ...
  const decisionsResult = await captureCheck(() => runDecisions('check', undefined, {}))
