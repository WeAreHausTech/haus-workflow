...
const ECOSYSTEM_PRIMARY_BACKENDS = new Set([
  'laravel',
  'wordpress',
  'vendure',
  'nestjs',
...
export async function recommend(
  ...
  const setupAnswers =
    (await readJson<Record<string, string>>(
      hausPath(root, 'setup-answers.json'),
    )) ?? {}
  const sources =
  ...
  const sourceTrust = new Map(
    (sources.items ?? []).map((x) => [x.id, x.status ?? 'candidate']),
  )
  ...
  for (const item of items) {
    ...
    if (configSignal) {
      pushReason(
        ...
        'config signal match',
        20,
        `warning:${configSignal}`,
      )
    }

    const changedMatch = changedFiles.find((f) =>
      f.includes(item.id.split('.').pop() ?? ''),
    )
    if (changedMatch) {
      pushReason(
        'changed-file-match',
        'changed file match',
        10,
        `changedFile:${changedMatch}`,
      )
    }

    if (
      item.id === 'haus.nx21-monorepo-patterns' &&
      !roleSet.has('nx-monorepo')
    ) {
      skipped.push({
        id: item.id,
    ...
    if (
      item.id === 'haus.turbo-monorepo-patterns' &&
    ...
    if (
      item.ecosystem &&
      dominantBackendEcosystem &&
      isBackendEcosystem(item.ecosystem)
    ) {
      const compat =
    ...
    if (score >= minScore) {
      const confidenceLevel = computeConfidenceLevel({
        isDefaultBaseline,
        reasons,
        hasEcosystemConflict: skipReasons.some(
          (s) => s.code === 'ecosystem-conflict',
        ),
        score,
      })
      ...
      recommended.push({
        ...
        selectionMode:
          isDefaultBaseline &&
          reasons.every((r) => r.code === 'default-baseline')
            ? 'baseline'
            : 'matched',
        install: true,
        score,
  ...
  const estimatedTokenReductionPct = Math.max(
    0,
    Math.round(
      (skippedRules / Math.max(selectedRules + skippedRules, 1)) * 100,
    ),
  )
  return {
    mode: context.mode,
...
function describeRequiresAny(clauses: RequiresAnyClause[]): string {
  return clauses
    .map((c) => {
      ...
      if ('packageNamePattern' in c)
        return `packageNamePattern=${c.packageNamePattern}`
...
function confidenceLevelToNumber(
