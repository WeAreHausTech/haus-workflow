...
export async function recommend(root: string, context: ContextMap): Promise<Recommendation> {
  ...
  const sources =
    (await readJson<{ items?: Array<{ id: string; status?: string }> }>(hausPath(root, "sources-report.json"))) ?? {};
  ...
  for (const item of items) {
    ...
    if (score >= minScore) {
      ...
      recommended.push({
        ...
        confidenceLevel,
        selectionMode:
          isDefaultBaseline && reasons.every((r) => r.code === "default-baseline") ? "baseline" : "matched",
        install: true,
        score,
        scoreBreakdown: {
          bonuses: reasons,
