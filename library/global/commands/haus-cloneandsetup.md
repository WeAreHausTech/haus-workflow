...
5. **Env (deterministic):** write each repo's `.env` from known values — no improvising.
   ...
   3. **Real secrets the generator can't mint** (third-party API keys, prod credentials, license keys) go in as clearly-marked `KEY=` blanks for the user to fill. **Never copy a secret value from another repo to populate them, and never record a secret in the workspace `localdev.yml` `env` map** — that map is for non-secret cross-repo literals only (DB names, ports, host URLs). The flow's job is to **instruct, not to propagate**: for every blank, tell the user exactly **which key**, **what it unlocks** (the step or feature that fails without it), and **where to obtain it**. If a sibling repo's `.env.example` already carries the same value under a different framework prefix (`MIX_` / `VITE_` / `NEXT_PUBLIC_` / `REACT_APP_`), name that file as the source so the user can copy it themselves — surface the location, never the value. Report all blanks together as one actionable list, not piecemeal.
