...
- **Databases and other services (a repo's `needs:`) run in Docker as a single workspace-owned `docker-compose` project — grouped, not separate `docker run` containers.** Author one compose project for the whole workspace (every repo's `needs:` service becomes a service in it) so the stack is self-contained and starts/stops together (`docker compose up -d` / `down`) with **named volumes** so data survives restarts. Pick image/internal-port/env from the house convention per service (e.g. `mysql` → `mysql:8`, root password from generated secrets; `postgres` → `postgres:16`; `mariadb`/`redis`/`elasticsearch` likewise). **Isolate per workspace instance** so the same workspace can be cloned and run multiple times side by side: set a unique `COMPOSE_PROJECT_NAME` (derived from the workspace folder/instance — namespaces containers, networks, and volumes) and give each instance its **own host ports**. **Do not pull a `needs:` service from the repo's own `docker-compose.yml` when that compose bind-mounts repo-relative init files** (e.g. `./seed.sql`, `./docker-entrypoint-initdb.d/`): those mounts need files the guard won't let us create, and they conflate bring-up with seeding (the separate `seed:` step) — author the service in the workspace compose instead. **If Docker isn't installed** it's a prerequisite (surfaced in the Step 2 gate) — **ask before installing it** (global install). **Host-port assignment:** host-run apps (e.g. Vendure `yarn dev`, the Valet PHP site) connect over `127.0.0.1:<port>`, so each instance needs distinct host ports — start from the conventional port, increment on conflict, publish them from the compose via per-service port vars, and **record the chosen ports in the workspace `localdev.yml` `env` map** so sink repos' `.env` point at the right place. A `needs:` service comes up **empty** — populating it is always the separate `seed:` step.
...
3. **Per repo, in order**, apply intent via the conventions:
   - **`needs`** → add each service to the **workspace `docker-compose` project** (image/internal-port/env from conventions; **not** the repo's compose when it bind-mounts repo-relative init files) and `docker compose up -d` if not already running — namespaced by `COMPOSE_PROJECT_NAME`, with free host ports assigned per instance. The service comes up **empty**; record its chosen host port/values in the workspace `localdev.yml` `env` map. No data is created here, so no overwrite prompt at this step — data lands in `seed:`.
